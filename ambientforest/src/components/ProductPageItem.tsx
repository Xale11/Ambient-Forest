  ...
  return (
    <HStack w={"90%"} h={{base: "unset", lg: "calc(100vh - 11em)"}} border={"1px solid #AD974F"} wrap={{base: "wrap", lg: "nowrap"}} >
      ...
      <VStack h={{base: "unset", lg: "100%"}} w={{base: "100%", lg: "50%"}} overflowY={"hidden"}>
        <Image h={{base: "unset", lg: "80%"}} w={{base: "100%", lg: "unset"}} maxH={"65vh"} aspectRatio={"1/1"}  objectFit={"contain"} src={`${baseImageUrl}/${product?.mainImageUrl}`} alt="Image of candle/tealight/giftset for sale"/>
      ...
      <VStack align={"start"} justify={{base: "center", "2xl": "start"}} h={{base: "unset", lg: "100%"}} w={{base: "100%", lg: "50%"}} gap={{base: 5, xl: 7}} pt={{base: 1, xl: 7}}>
        ...
        <HStack w={"100%"} px={2} display={{base: "flex", lg: "none"}}>
          <Link to={"/cart"} style={{width: "50%"}}>
            <Button onClick={() => addToBag(product)} fontSize={"md"} color={"white"} bg={"--black"} w={"100%"} transition={"all 300ms ease-in-out"}>
              Add To Basket
            </Button>
          </Link>
          <Link to={"/cart"} style={{width: "50%"}}>
        ...
        <HStack w={"100%"} display={{base: "none", lg: "flex"}}>
          <Link to={"/cart"}>
            <Button onClick={() => addToBag(product)} fontSize={"md"} color={"white"} bg={"--black"} w={"13em"} _hover={{w: "15em"}} transition={"all 300ms ease-in-out"}>
              Add To Basket
            </Button>
          </Link>
          <Link to={"/cart"}>
